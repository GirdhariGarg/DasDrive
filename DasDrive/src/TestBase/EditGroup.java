...
public class EditGroup extends BaseClass {

	String para3;
	String TDSheet = "EditGroup";
	@Test
	public void validate(int row) throws IOException, InterruptedException {
		...
		case "y":
			...
			case "GE_01": // validate test case
				driver.findElement(By.xpath(editgroupb)).click();
				driver.manage().timeouts().implicitlyWait(60, TimeUnit.SECONDS);
				driver.findElement(By.id(groupdescboxid)).clear();
				driver.findElement(By.id(groupdescboxid)).sendKeys(parameter3);
			...
			case "GE_03": // validate test case
				int j = 2;
				String[] ff = expectedresult.split(",");
				...
				for(int i  = 0; i<=ff.length-1; i++){
					driver.findElement(By.xpath(grouppopp1+j+grouppopp2)).click();
					System.out.println(driver.findElement(By.xpath(grouppopvalue1+j+grouppopvalue2)).getText());
				...
				xltcdata.setCellData(TDSheet, TDSspath, row, spath);
				driver.manage().timeouts().implicitlyWait(60, TimeUnit.SECONDS);
				Assert.assertEquals(driver.findElement(By.xpath(grouppopvalue1+j+grouppopvalue2)).getText().contains(ff[i]), true, "" + xltcdata.setCellData(TDSheet, TDResult, row,"Fail"));
				xltcdata.setCellData(TDSheet, TDResult, row, "Pass");
				System.out.println("TestCase Passed With Expected Reuslt" + expectedresult);
				j++;
				}
